...
public class ClientApplication {
        ...
        try {
            if (caseArgs.length == 1) {
                ...
                if (value.compareTo(BigInteger.ZERO) < 0) {
                    System.out.println("[ERROR] Transaction value cannot be negative.");
                    return;
                }
            }
        } catch (NumberFormatException e) {
    ...
    private void handleSmartContractExecution(Scanner scanner, ClientLibrary clientLibrary) throws Exception {
        ...
        JSONObject calls = helpers.loadJsonFromFile(
                Constants.CALLS_FILE);
        System.out.println(
                "Welcome to Contract Execution ! Enter <help> for the available contract calls , or 'exit' to quit:");
        while (true) {
            System.out.print("> ");
            String input = scanner.nextLine().trim();
            ...
            switch (caseArgs[0].toLowerCase()) {
                ...
                case Constants.ExecType.TRANSFER:
                    ...
                    if (caseArgs.length != 4) { // transfer <contractAddress> [<data>]
                        System.out
                                .println("[ERROR] Please provide the command: transfer <contractAddress> <to> <value>");
                        break;
                    }
                    BigInteger value = new BigInteger(caseArgs[3]);
                    String finalData = data + helpers.padHexStringTo256Bit(caseArgs[2])
                            + helpers.convertBigIntegerToHex256Bit(value);
                ...
                case Constants.ExecType.APPROVE:
                    ...
                    if (caseArgs.length != 4) { // approve <contractAddress> [<data>]
                        System.out
                                .println("[ERROR] Please provide the command: approve <contractAddress> <to> <value>");
                        break;
                    }
                ...
                case Constants.ExecType.TRANSFERFROM:
                    ...
                    if (caseArgs.length != 5) { // transferFrom <contractAddress> [<data>]
                        System.out.println(
                                "[ERROR] Please provide the command: transferFrom <contractAddress> <from> <to> <value>");
                        break;
                    }
                    value = new BigInteger(caseArgs[4]);
                    finalData = data + helpers.padHexStringTo256Bit(caseArgs[2])
                            + helpers.padHexStringTo256Bit(caseArgs[3]) + helpers.convertBigIntegerToHex256Bit(value);
                    System.out.println("[INFO] data: " + finalData);
                    SmartContractExecutionRequest(caseArgs, clientLibrary, finalData);
                    break;
                ...
                case Constants.ExecType.ISBLACKEDLISTED:
                    ...
                    if (caseArgs.length != 3) { // isBlacklisted <contractAddress> [<data>]
                        System.out.println(
                                "[ERROR] Please provide the command: isBlacklisted <contractAddress> <address>");
                        break;
                    }
                    finalData = data + helpers.padHexStringTo256Bit(caseArgs[2]);
                    System.out.println("[INFO] data: " + finalData);
                ...
                case Constants.ExecType.ADDTOBLACKLIST:
                    ...
                    if (caseArgs.length != 3) { // addBlacklist <contractAddress> [<data>]
                        System.out.println(
                                "[ERROR] Please provide the command: addToBlacklist <contractAddress> <address>");
                        break;
                    }
                    finalData = data + helpers.padHexStringTo256Bit(caseArgs[2]);
                    System.out.println("[INFO] data: " + finalData);
                ...
                case Constants.ExecType.REMOVEFROMBLACKLIST:
                    ...
                    if (caseArgs.length != 3) { // removeFromBlacklist <contractAddress> [<data>]
                        System.out.println(
                                "[ERROR] Please provide the command: removeFromBlacklist <contractAddress> <address>");
                        break;
                    }
                    finalData = data + helpers.padHexStringTo256Bit(caseArgs[2]);
                    System.out.println("[INFO] data: " + finalData);
                ...
                case "help":
                    System.out.println(
                            "[INFO] Available contract calls: transfer, approve, transferFrom, isBlacklisted, addToBlacklist, removeFromBlacklist");
                    break;
                default:
    ...
    private void SmartContractExecutionRequest(String[] caseArgs, ClientLibrary clientLibrary, String data)
            throws Exception {
